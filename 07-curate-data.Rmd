...
```{r cd-regex-101-image, echo=FALSE, fig.cap='RegEx101.'}
knitr::include_graphics("images/07-curate-data/cd-regex-101.png")
```
...
```{r cd-semi-extract-sdac-metadata-function}
...
```{r cd-extract-sdac-metadata-test-show, eval=FALSE}
extract_sdac_metadata(file = "../data/original/sdac/sw00utt/sw_0001_4325.utt") %>% 
  glimpse()
```

```{r cd-extract-sdac-metadata-test-run, echo=FALSE}
extract_sdac_metadata(file = "resources/07-curate-data/sdac/sw00utt/sw_0001_4325.utt") %>% 
  glimpse()
```
...
```{r cd-semi-sdac-list-files-show, eval=FALSE}
sdac_files <- 
...
```{r cd-semi-sdac-list-files-run, echo=FALSE, results='hide'}
sdac_files <- 
  fs::dir_ls(path = "resources/07-curate-data/sdac/", # source directory
             recurse = TRUE, # traverse all sub-directories
             type = "file", # only return files
             regexp = "\\.utt$") # only return files ending in .utt
head(sdac_files)
...
```{r cd-semi-sdac-tidy-all, results='hide'}
...
We now see that we have `r nrow(sdac)` observations (individual utterances in this dataset). 

```{r cd-semi-sdac-preview-complete}
...
```{r cd-semi-sdac-write, eval=FALSE}
...
```{r cd-semi-sdac-summary-code, eval=FALSE}
...
```{r cd-documentation-dic-starter-function, eval=FALSE}
...
```{r cd-documentation-dic-starter-structure, echo=FALSE}
tibble(variable_name = names(sdac), name = "", description = "") %>% 
  knitr::kable(booktabs = TRUE, 
               caption = "Data dictionary starter structure for the SDAC curated dataset.")
```
...
```{r cd-documentation-starter-edit, echo=FALSE}
knitr::include_graphics("images/07-curate-data/cd-data-dictionary.png")
```
