...
```{r summaries-multivariate-scatterplot-belc, echo=FALSE, fig.cap='Scatterplot visualizing the relationship between `num_tokens` and `ttr`.'}
p1 <- 
  belc %>% 
...
```{r summaries-bivariate-barplot-belc, echo=FALSE, fig.cap='Barplot comparing the mean `num_tokens` by `age_group` from the BELC dataset.'}
p1 <- 
  belc %>% 
  group_by(age_group) %>% 
  summarize(mean_num_tokens = mean(num_tokens),
