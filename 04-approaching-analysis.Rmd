...
# Packages
pacman::p_load(TBDBr, janitor, skimr)
```
...
```{r summaries-bivariate-barplot-belc, echo=FALSE, fig.cap='Barplot...'}
p1 <- 
  belc %>% 
  group_by(age_group) %>% 
  summarize(mean_num_tokens = mean(num_tokens),
...
```{r summaries-multivariate-barplot-belc, echo=FALSE, fig.cap='Barplot of the relationship between `age_group`, `num_tokens` and `sex` from the BELC dataset.'}
belc %>% 
...
```{r summaries-bivariate-boxplots-belc, echo=FALSE, fig.cap='Boxplot of the relationship between `age_group`, `num_tokens` and `sex` from the BELC dataset.'}
p1 <- 
  belc %>% 
...
```{r summaries-multivariate-boxplots-belc, echo=FALSE, fig.cap='Boxplot for multivariate relationship between `age_group`, `num_tokens`, and `sex`.'}
  belc %>% 
...
```{r summaries-bivariate-categorical-table-belc}
# ...
belc %>% 
  tabyl(sex, age_group) %>% 
  kable(booktabs = TRUE,
        caption = "Crosstabulation table for `age_group` and `sex`.")
```

  - association (Cramer's V?)

```{r summaries-bivariate-categorical-barplot-belc}
# ...
...
```{r summaries-multivariate-categorical-table-belc}
# ...
...
belc %>% 
  ...
  kable(booktabs = TRUE,
        caption = "Crosstabulation table for `age_group`, `rank_tokens`, and `sex`.")
```
...
```{r summaries-multivariate-mosaic-belc}
# ...
