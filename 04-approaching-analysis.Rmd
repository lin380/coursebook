...
```{r approaching-analysis-packages, echo=FALSE}
# Packages
pacman::p_load(TBDBr, janitor, skimr, effectsize, kableExtra)
```
...
```{r tbdbr-dataset-belc-compositions, echo=FALSE, results='hide'}
...
belc %>% 
  slice_head(n =10) %>% 
  kable(booktabs = TRUE,
        caption = 'First 10 observations of the BELC dataset for demonstration.')
```
...
```{r info-values-graphic, echo=FALSE, fig.cap='Levels of measurement graphic representation.'}
knitr::include_graphics("images/04-approaching-analysis/Informational-values-paper.png")
```
...
```{r summaries-boxplots-belc, echo=FALSE, fig.cap='Boxplots for each of the continuous variables in the BELC dataset.'}
p1 <- 
  belc %>% 
  ggplot(aes(y = num_tokens)) + 
  geom_boxplot(outlier.colour = "red") + 
  theme(axis.title.x=element_blank(), axis.text.x = element_blank()) +
  labs(y = "Number of tokens")

p2 <- 
  belc %>% 
  ggplot(aes(y = num_types)) + 
  geom_boxplot(outlier.colour = "red") + 
  theme(axis.title.x=element_blank(), axis.text.x = element_blank()) +
...
```{r summaries-histograms-belc, echo=FALSE, fig.cap='Histograms and density plots for the continuous variables in the BELC dataset.'}
p1 <- 
  belc %>% 
...
```{r summaries-skew-graphic, echo=FALSE, out.width='100%', fig.cap='Examples of skew types in density plots.'}
knitr::include_graphics("images/04-approaching-analysis/skew-types-paper.png")
```
...
```{r summaries-qqnorm-plot-belc, echo=FALSE, fig.cap='QQ Plots for the continuous variables in the BELC dataset.'}
p1 <- 
  belc %>% 
...
```{r summaries-normality-test-belc, echo=FALSE}
...
t %>% 
  ...
  kable(booktabs = TRUE,
        caption = "Results from Shapiro-Wilk test of normality for continuous variables in the BELC dataset.")
```
...
```{r summarize-ecdf-belc, echo=FALSE, fig.cap='ECDF plots for the continuous variables in the BELC dataset.'}
p1 <- 
  belc %>% 
  ggplot(aes(x = num_tokens)) +
  stat_ecdf(geom = "step") +
...
```{r summaries-categorical-measures-belc, echo=FALSE}
belc %>% 
...
```{r summaries-bivariate-scatterplot-belc, echo=FALSE, fig.cap='Scatterplot...'}
p1 <- 
  belc %>% 
...
```{r summaries-multivariate-scatterplot-belc, echo=FALSE, fig.cap='Scatterplot visualizing the relationship between `num\\_tokens` and `ttr`.'}
p1 <- 
  belc %>% 
...
```{r summarize-bivariate-cont-cat-table, echo=FALSE}
belc %>% 
  group_by(age_group) %>% 
...
```{r summaries-bivariate-barplot-belc, echo=FALSE, fig.cap='Barplot comparing the mean `num\\_tokens` by `age\\_group` from the BELC dataset.'}
p1 <- 
  belc %>% 
  group_by(age_group) %>% 
  summarize(mean_num_tokens = mean(num_tokens),
...
To gauge the effect size of this relationship we can use _Spearman's rho_ for rank-based coefficients. The score is `r cor(as.numeric(belc$age_group), belc$num_tokens, method = "spearman")` indicating that the relationship between `age_group` and `num_tokens` is quite strong. ^[To calculate effect sizes for the difference between two means, _Cohen's d_ is used.]
...
```{r summaries-multivariate-barplot-belc, echo=FALSE, fig.cap='Barplot comparing the mean `num_tokens` by `age_group` and `sex` from the BELC dataset.'}
belc %>% 
...
```{r summaries-bivariate-boxplots-belc, echo=FALSE, fig.cap='Boxplot of the relationship between `age_group` and `num_tokens` from the BELC dataset.'}
p1 <- 
  belc %>% 
...
```{r summaries-multivariate-boxplots-belc, echo=FALSE, fig.cap='Boxplot of the relationship between `age_group`, `num_tokens` and `sex` from the BELC dataset.'}
  belc %>% 
...
```{r summaries-bivariate-categorical-table-belc, echo=FALSE}
# ...
belc %>% 
  tabyl(sex, age_group) %>% 
...
```{r summaries-bivariate-categorical-barplot-belc, echo=FALSE, fig.cap='Barplot...'}
p1 <- 
  belc %>% 
...
```{r summaries-multivariate-categorical-table-belc-female, echo=FALSE}
tab <- 
  belc %>% 
...
```{r summaries-multivariate-categorical-table-belc-male, echo=FALSE}
tab$male %>% 
    kable(booktabs = TRUE,
...
```{r summaries-multivariate-mosaic-belc, echo=FALSE, fig.cap='Mosaic plot for three categorical variables `age_group`, `rank_tokens`, and `sex` in the BELC dataset.'}
# ...
...
```{r aa-inferential-variables, echo=FALSE, fig.cap='Variable roles in inferential analysis.'}
knitr::include_graphics("images/04-approaching-analysis/inferential-variables.png")
```
...
```{r ida-statistical-monofactorial-listing, echo=FALSE}
monofactorial <- 
  tribble(
...
monofactorial %>% 
  knitr::kable(booktabs = TRUE,
             caption = "Common monofactorial tests.") %>% 
...
```{r ida-statistical-multifactorial-listing, echo=FALSE}
multifactorial <- 
  tribble(
...
multifactorial %>% 
  knitr::kable(booktabs = TRUE,
             caption = "Common multifactorial tests.") %>% 
...
```{r pda-sms-data, echo=FALSE}
sms <- tadr::sms
sms_n_rows <- nrow(sms)
```

To give an example, let's consider a language task in which the goal is to take text messages (SMS) and develop a language model that predict if a message is spam or not. Minimally we would need data which includes individual text messages and each of these text message will need to be labeled as being either spam or legitimate messages ('ham' in this case). In Table \@ref(tab:pda-sms-preview) we see the first ten of `r sms_n_rows` observations from the SMS Spam Collection (v.1) dataset collected by @Almeida2011a.  

```{r pda-sms-preview, echo=FALSE}
sms %>% 
  slice_head(n = 10) %>% 
  kable(booktabs = TRUE,
        caption = 'First ten observations from the SMS Spam Collection (v.1)')
```
...
```{r aa-predictive-variables, echo=FALSE, fig.cap='Variable roles in predictive analysis.'}
knitr::include_graphics("images/04-approaching-analysis/predictive-variables.png")
```
...
algorithms_listing %>% 
  kable(booktabs = TRUE,
        caption = 'Some common supervised learning algorithms.')
```


```{r pda-classification-algorithms, echo=FALSE, eval=FALSE}
...
classification_listing %>% 
  kable(booktabs = TRUE,
        caption = 'Common classification algorithms.')

```


```{r pda-regression-algorithms, echo=FALSE, eval=FALSE}
regression_listing <- 
  tribble(
    ~"Name", ~"Advantages", ~"Shortcomings",
    "Linear Regresssion", "...", "...",
...
regression_listing %>% 
  kable(booktabs = TRUE,
        caption = 'Common regression algorithms.')
```
...
```{r aa-predictive-phases, echo=FALSE, fig.cap='Phases in predictive analysis.'}
knitr::include_graphics("images/04-approaching-analysis/predictive-phases.png")
```
...
```{r aa-exploratory-variables, echo=FALSE, fig.cap='Variable roles in exploratory analysis.'}
knitr::include_graphics("images/04-approaching-analysis/exploratory-variables.png")
```
...
```{r eda-sotu-corpus, echo=FALSE}
library(quanteda)
sotu_corpus <- 
  quanteda.corpora::data_corpus_sotu %>% 
  corpus_subset(Date > "1946-01-01")


data.frame(docvars(sotu_corpus[1:10]), 
           addresses = as.character(sotu_corpus[1:10])) %>% 
  select(president = President,
         date = Date,
         delivery, party, addresses) %>% 
  mutate(addresses = str_trunc(addresses, 50, "right")) %>% 
  kable(booktabs = TRUE, row.names = FALSE,
        caption = "First ten addresses from the SOTU Corpus.")
```
...
```{r eda-approaches-table, echo=FALSE}
eda_approaches <- 
  tribble(
...
eda_approaches %>% 
  kable(booktabs = TRUE,
      caption = "Some common EDA analysis methods.")
```
...
p1 <- # Frequency plots
  ...
  dfm() %>% 
  textstat_frequency(n = 20, groups = party) %>% 
...
```{r eda-sotu-keyness, echo=FALSE}
p2 <- # Barplot: keyness
  sotu_corpus %>% 
  tokens(remove_punct = TRUE) %>% 
  tokens_remove(pattern = stopwords("en")) %>% 
  dfm() %>% 
  textstat_keyness(target = .$Date > "2001-09-10") %>% 
...
```{r eda-sotu-collocation, echo=FALSE}
...
sotu_df <- 
  data.frame(docvars(sotu_corpus), address = as.character(sotu_corpus))
...
p3 <- 
    ...
    theme_void() +
    labs(title = "N-grams with 'Free'")

```

```{r eda-sotu-clustering, echo=FALSE}
...
```{r eda-visualizations-grid, echo=FALSE, fig.cap='Sample visualizations from the SOTU Corpus (1946-2020).'}
# Visualization grid
...
```{r approaching-analysis-visual-summary-graphic, echo=FALSE, fig.cap='Approaching analysis: visual summary'}
knitr::include_graphics("images/04-approaching-analysis/approaching-analysis-visual-summary-paper.png")
```
